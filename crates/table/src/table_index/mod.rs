...
impl<'a> TypedIndexKey<'a> {
    ...
    /// driven by the kind of [`TypedIndex`] provided in `index`.
    #[inline]
    fn from_bsatn(index: &TypedIndex, ty: &AlgebraicType, mut bytes: &'a [u8]) -> Result<Self, DecodeError> {
...
impl TableIndex {
    ...
    fn bounds_from_bsatn(
        ...
        rstart: &'a [u8],
        rend: &'a [u8],
    ) -> Result<PointOrRange<'a>, DecodeError> {
        use TypedIndex::*;
        ...
        match key_type {
            ...
            AlgebraicType::Product(key_types) => {
                ...
                match idx {
                    ...
                    BTreeAV(_) | HashAV(_) | UniqueBTreeAV(_) | UniqueHashAV(_) => {
                        ...
                        if let Some(mut point) = Self::as_point_scan(&start, &end, suffix_len) {
                            let point = AlgebraicValue::decode(&range_type.algebraic_type, &mut point)?;
                            let point = Self::combine_prefix_and_point(prefix, point);
                            return Ok(PointOrRange::Point(from_av(point)));
                        }

                        // It's not a point scan.
                        let decode_bound = |b: Bound<_>| {
                            transpose_bound(b.map(|mut b| AlgebraicValue::decode(&range_type.algebraic_type, &mut b)))
                        };
            ...
            ty if prefix.is_empty() && prefix_elems.idx() == 0 => {
                if let Some(point) = Self::as_point_scan(&start, &end, 0) {
                    let point = Self::from_bsatn(idx, ty, point)?.into();
                    return Ok(PointOrRange::Point(point));
                }

                let decode = |b: Bound<_>| transpose_bound(b.map(|b| Self::from_bsatn(idx, ty, b).map(Into::into)));
    ...
    ) -> Result<PointOrRange<'de>, DecodeError> {
        // Is this really a point scan?
        let from = |k| ctor(k).into();
        Ok(if let Some(point) = Self::as_point_scan(&start, &end, suffix_len) {
            let point = BytesKey::from_bsatn_prefix_and_endpoint(prefix, point)?;
            PointOrRange::Point(from(point))
        } else {
            // It's not a point scan.
            let decode_bound = |b: Bound<&[u8]>| {
                transpose_bound(b.map(|b| BytesKey::from_bsatn_prefix_and_endpoint(prefix, b).map(from)))
            };
